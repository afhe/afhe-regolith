...
apply() {
  ...
  local \
    color0="$($resource_getter gnome.terminal.color0)" \
    color1="$($resource_getter gnome.terminal.color1)" \
    color2="$($resource_getter gnome.terminal.color2)" \
    color3="$($resource_getter gnome.terminal.color3)" \
    color4="$($resource_getter gnome.terminal.color4)" \
    color5="$($resource_getter gnome.terminal.color5)" \
    color6="$($resource_getter gnome.terminal.color6)" \
    color7="$($resource_getter gnome.terminal.color7)" \
    color8="$($resource_getter gnome.terminal.color8)" \
    color9="$($resource_getter gnome.terminal.color9)" \
    color10="$($resource_getter gnome.terminal.color10)" \
    color11="$($resource_getter gnome.terminal.color11)" \
    color12="$($resource_getter gnome.terminal.color12)" \
    color13="$($resource_getter gnome.terminal.color13)" \
    color14="$($resource_getter gnome.terminal.color14)" \
    color15="$($resource_getter gnome.terminal.color15)" \
    font="$($resource_getter gnome.terminal.font "monospace")" \
    scrollbar="$($resource_getter gnome.terminal.scrollbar never)" \
    useTransparentBg="$($resource_getter gnome.terminal.use-transparent-background false)" \
    audibleBell="$($resource_getter gnome.terminal.audible-bell false)" \
    transparentBgPercent="$($resource_getter gnome.terminal.background-transparency-percent 4)" \
    blink="$($resource_getter gnome.terminal.cursor-blink-mode off)"
  ...
  local background="$($resource_getter gnome.terminal.background-color $color0)"
  local foreground="$($resource_getter gnome.terminal.foreground-color $color7)"
  ...
  log 4 "Set Regolith GNOME Terminal version key of the '$profile_name' profile"
}

# Applies the Regolith GNOME Terminal theme with version comparison.
#
...
apply_version_compared() {
  ...
  case "$(vercomp "$NORD_GNOME_TERMINAL_VERSION" "$version"; echo $?)" in
    0)
      log 3 "Reinstalling Regolith GNOME Terminal since the version equaled the version of the '$profile_name' profile!";
      apply;
      log 2 "Reinstalled Regolith GNOME Terminal version $NORD_GNOME_TERMINAL_VERSION for the '$profile_name' profile!";
      exit 0;;
    1)
      ...
      exit 0;;
    2)
      log 1 "The detected Regolith GNOME Terminal version $version of the '$profile_name' profile is greater than the version that will be applied!";
      ...
      case $confirmation in
        [Yy]* )
          apply;
          log 2 "Regolith GNOME Terminal version $NORD_GNOME_TERMINAL_VERSION has been successfully applied!";
          exit 0;;
        [Nn]* )
...
# Gets the current Regolith GNOME Terminal version.
#
# @globread NORD_GNOME_TERMINAL_VERSION_DCONF_KEY DCONF_PROFILE_BASE_PATH # @param $1 the UUID of the profile to check
# @since 0.2.0
get_current_version() {
  local uuid version
  uuid=$1
...
NORD_PROFILE_VISIBLE_NAME="Regolith"
log_level=2
...
if validate_dependencies DEPENDENCIES[@]; then
  ...
  if [[ -n $profile_name ]]; then
    ...
    if [[ -n $profile_uuid ]]; then
      ...
      if [[ -n $curr_ver ]]; then
        ...
        apply
        log 2 "Regolith GNOME Terminal version $NORD_GNOME_TERMINAL_VERSION has been successfully applied to the '$profile_name' profile"
        exit 0
      fi
    else
  ...
  # Check for an already existing 'Regolith' profile and update it
  ...
  clone_default_profile
  apply
  log 2 "Regolith GNOME Terminal version $NORD_GNOME_TERMINAL_VERSION has been successfully applied to the newly created '$NORD_PROFILE_VISIBLE_NAME' profile"
  exit 0
else
